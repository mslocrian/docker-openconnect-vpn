...
class SAMLLoginView:
    def __init__(self, uri, html=None, verbose=False, cookies=None, verify=True):
        ...
        self.view = QWebView()
        self.view.setWindowTitle("SAML Login")
        self.view.setFixedSize(500, 600)
        ...
        res = self.view.setHtml(html)
        self.view.show()
        x = self.view.page()
        nam = x.networkAccessManager()
        nam.finished.connect(self.handle_finish)

    def handle_finish(self, reply):
        # reply.waitForReadyRead(5)
        resp = reply.readAll()
        ...
        if 'Login Successful' in resp:
            ...
            for h in headers:
                rh = reply.rawHeader(h)
                self.headers[h.data().decode()] = rh.data().decode()
            self.view.close()
            QCoreApplication.quit()
            Gtk.main_quit()
            self.success = True
            self.get_saml_headers()
            return
        else:
            self.success = False
            self.closed = True
    ...
    def get_saml_headers(self):
        ...
        fd = {name:v for name, v in self.headers.items() if name.startswith('saml-') or name in ('prelogin-cookie', 'portal-userauthcookie')}
        if fd and self.verbose:
            ...
            if self.verbose > 1:
                ...
                mr.get_data(None, self.log_resource_text, ct[0], ct.params.get('charset'), self.headers)
    ...
    def get_saml_headers_OLD(self, webview, event):
        ...
        # check if we're done
        self.saml_result.update(fd, server=urlparse(uri).netloc)
        print(f"d={d} saml_result={self.saml_result}")
        GLib.timeout_add(1000, self.check_done)
...
def main(args = None):
    ...
    if args.exec:
        ...
        print('\n'.join('export %s=%s' % pair for pair in varvals.items()))
